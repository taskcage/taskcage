...
- PR 본문에는 아래 내용을 모두 작성합니다. 해당 내용이 없으면 `없음`이라고 적습니다.
...
- README는 외부 사용자를 위한 짧은 소개와 시작점으로 유지합니다.
- 긴 설계 설명과 운영 가정은 `docs/` 아래에 둡니다.
- cgroup 격리, IPC 프로토콜, 권한 모델, 공개 SDK API처럼 큰 기술적 판단은 `docs/decisions/`에 기록합니다.
- 결정 문서에는 문제, 선택지, 선택 이유, 검증 방법, 남은 위험을 적고 관련 Issue와 PR을 연결합니다.
