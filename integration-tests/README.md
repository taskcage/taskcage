...
Core Profile API와 generic FFmpeg Capsule 실행까지 함께 검증한다.
...
이 검증은 폐기 가능한 PGP key로 Java Core SDK Central bundle을 만든다. bundle만 임시 Maven repository에
풀고, 저장소 composite build를 사용하지 않는 별도 Java 17 project가 Core SDK를 resolve해 compile하는지
확인한다. 공개 registry에는 아무것도 upload하지 않는다.
...
publish하는지도 확인한다. 실제 generic Java Capsule E2E는 container workflow에서 별도로 검증한다.
