...
#[cfg(target_os = "linux")]
enum BundleCommand {
    Import(BundleImportConfig),
    List(BundleListConfig),
    Inspect(BundleInspectConfig),
}
...
#[cfg(target_os = "linux")]
struct BundleListConfig {
    cache_root: PathBuf,
}

#[cfg(target_os = "linux")]
struct BundleInspectConfig {
    cache_root: PathBuf,
    name: String,
    version: String,
}
...
#[cfg(target_os = "linux")]
fn parse_bundle_command(args: Vec<OsString>) -> taskcaged::Result<BundleCommand> {
    ...
    match subcommand {
        "import" => parse_bundle_import(options).map(BundleCommand::Import),
        "list" => parse_bundle_list(options).map(BundleCommand::List),
        "inspect" => parse_bundle_inspect(options).map(BundleCommand::Inspect),
...
#[cfg(target_os = "linux")]
fn parse_bundle_import(args: &[OsString]) -> taskcaged::Result<BundleImportConfig> {
    let mut source = None;
...
#[cfg(target_os = "linux")]
fn parse_bundle_list(args: &[OsString]) -> taskcaged::Result<BundleListConfig> {
    let mut cache_root = None;
    parse_single_bundle_option(args, "list", &mut cache_root, None, None)?;
    let cache_root = required_option("cache-root", cache_root)?;
    ...
    Ok(BundleListConfig { cache_root })
}

#[cfg(target_os = "linux")]
fn parse_bundle_inspect(args: &[OsString]) -> taskcaged::Result<BundleInspectConfig> {
    let mut cache_root = None;
    let mut name = None;
    let mut version = None;
    parse_single_bundle_option(
        args,
        "inspect",
        &mut cache_root,
        Some(&mut name),
