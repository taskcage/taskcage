...
pub(crate) fn parse(args: Vec<OsString>) -> taskcaged::Result<RunOnceConfig> {
...
pub(crate) async fn execute(config: RunOnceConfig) -> taskcaged::Result<()> {
...
mod tests {
    ...
    fn working_directory_is_not_defaulted() {
        ...
        let error = parse(vec![OsString::from("--"), program.into_os_string()]).unwrap_err();
    ...
    fn rejects_relative_program_before_execution() {
        let error = parse(vec![OsString::from("--"), OsString::from("echo")]).unwrap_err();
