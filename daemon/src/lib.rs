...
pub async fn run_once(config: RunOnceConfig) -> Result<RunOnceReport> {
    // 명령 준비나 job cgroup 생성보다 먼저 실제 환경 검사를 모두 끝낸다.
    let probe = match &config.cgroup_root {
    ...
    tracing::info!(job_id = %config.job_id, pid = process.pid(), "target을 작업 cgroup 안에서 시작했습니다");
