...
- daemon은 socket을 위해 UID 또는 GID를 변경하지 않는다. 서비스 계정과 상위 디렉터리 권한은 배포
  설정의 책임이다.
- bind 전에 경로가 일반 파일, 디렉터리, symlink, 활성 socket 또는 stale socket으로 존재하면 시작을
  거절한다. 어떤 기존 경로도 임의로 삭제하거나 덮어쓰지 않는다.
- 정상적으로 bind한 socket의 device와 inode를 기억한다. 종료 시 같은 socket인 경우에만 제거하며,
  경로가 바뀌었거나 다른 파일로 교체됐으면 삭제하지 않는다.
- crash 뒤 남은 stale socket의 판정과 제거는 startup recovery 작업으로 남긴다.
...
- **좁은 MVP 범위:** stale socket 자동 복구와 인증 protocol을 이번 서버 구현에 섞지 않는다.

## 검증 방법

- 상대 경로를 거절한다.
- bind 뒤 socket mode가 `0600`인지 확인한다.
- 기존 일반 파일, symlink와 socket을 삭제하지 않고 시작을 거절한다.
...
- crash 뒤 stale socket이 남으면 다음 시작은 fail-closed로 거절된다. 자동 판정과 제거는 startup
  recovery에서 구현한다.
