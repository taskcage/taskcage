...
첫 분리 작업에서는 backend-independent한 `CapsuleIdentity`를 `taskcage-core`로 옮겨 daemon이 이를
공유하도록 경계를 먼저 만든다. cgroup·프로세스 실행 모듈은 이 경계를 유지하면서 후속 커밋에서
점진적으로 이동한다. 따라서 이 단계만으로 EmbeddedRunner가 실행 가능해졌다고 간주하지 않는다.
